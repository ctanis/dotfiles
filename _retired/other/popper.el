;;; -*-Emacs-Lisp-*-
;;; %Header
;;; Shrink-wrapped temporary windows for GNU Emacs $Revision: 1.2 $
...
(defun popper-scroll-filter (process output)
  ...
  (let* ((old (selected-window))
    ...
    (if (and window)
	(progn
	  ;(setq *w window)        ;; commented out JSJ Thu Jun 18 17:01:35 CDT 1998
	  (select-window window)
	  (goto-char (point-max))
...
optional NO-ERROR is T."
  (interactive)
  (save-excursion ;; added ctanis 1-22-10
...
(defun pop-to-buffer (buffer &optional other-window norecord) ; added 3rd arg JSJ Thu Jun 18 17:14:54 CDT 1998
...
window even if BUFFER is already visible in the selected window."
  (let ((parent (popper-parent)))
	...
	(progn
	  (popper-bury-output)
	  (funcall popper-pop-to-buffer buffer other-window norecord)
...
;;;
(setq temp-buffer-show-function 'popper-show) ; JSJ
